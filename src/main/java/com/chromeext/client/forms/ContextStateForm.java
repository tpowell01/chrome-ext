...
public class ContextStateForm extends BaseForm {
    ...
    private void parseDataAndFillForm(JSONObject data) {
        ...
        String numberOfPages = data.get("number_of_pages").isString().stringValue();
        lblNumberOfPages.setText(numberOfPages);

        String elementCount = data.get("element_count").isString().stringValue();
        ...
        JSONObject actions = data.get("actions").isArray().get(0).isObject();
